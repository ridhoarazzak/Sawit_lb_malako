...
export default function SawitMap() {
  ...
  useEffect(() => {
    fetch(sawitGeojsonUrl)
      .then(res => res.json())
      .then(data => {
        const total = data.features.reduce((sum, f) => sum + (f.properties.luas || 0), 0);
        setTotalLuas(total);
      });
  }, []);

  const sawitLayer = new GeoJsonLayer({
    id: 'sawit-aktif',
    data: sawitGeojsonUrl,
    filled: true,
    stroked: true,
    visible: showLayer,
  ...
  const downloadGeoJSON = () => {
    ...
    link.href = sawitGeojsonUrl;
    link.download = 'kebun_sawit_aktif.geojson';
