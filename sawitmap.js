import React, { useState, useCallback, useEffect } from 'react'; import { DeckGL } from '@deck.gl/react'; import { StaticMap } from 'react-map-gl'; import { GeoJsonLayer } from '@deck.gl/layers';

const MAPBOX_TOKEN = 'pk.eyJ1IjoibWFwYm94dXNlciIsImEiOiJja2ZxZ2E3N2swMDVyMnBtcDN6cGZwbDBhIn0.1234567890abcdef'; // Ganti dengan tokenmu
...
const sawitGeojsonUrl = 'https://raw.githubusercontent.com/ridhoarazzak/Sawit_lb_malako/main/data/shp_sawit_lb_malako.geojson'; const batasNagariUrl = 'https://raw.githubusercontent.com/ridhoarazzak/Sawit_lb_malako/main/data/shp_nagari_lb_malako.geojson';

useEffect(() => { fetch(sawitGeojsonUrl) .then(res => res.json()) .then(data => { const total = data.features.reduce((sum, f) => sum + (f.properties.luas || 0), 0); setTotalLuas(total); }); }, []);

const sawitLayer = new GeoJsonLayer({ id: 'sawit-aktif', data: sawitGeojsonUrl, filled: true, stroked: true, visible: showLayer, getFillColor: [34, 139, 34, 180], getLineColor: [0, 0, 0, 100], lineWidthMinPixels: 1, pickable: true, autoHighlight: true, onHover: onHover, onClick: ({object}) => alert(Luas: ${object.properties.luas} m2) });
...
const downloadGeoJSON = () => { const link = document.createElement('a'); link.href = sawitGeojsonUrl; link.download = 'kebun_sawit_aktif.geojson'; document.body.appendChild(link); link.click(); document.body.removeChild(link); };
...
<div className="absolute top-4 left-4 bg-white p-3 rounded shadow z-10 w-60">
